...
window.onload = function () {
    window.startLoader = function () {
        $(".vcardpro-loader").show();
    };

    window.stopLoader = function () {
        $(".vcardpro-loader").hide();
    };
...
function frontTestimonials() {
    if ($(".pricing-carousel").length > 0) {
        ...
        $(".testimonial-carousel").slick({
            ...
            slidesToScroll: 1,
            arrows: false,
            rtl: frontLanguage == 'ar' || frontLanguage == 'fa',
            responsive: [
                {
                    breakpoint: 1199,
                    settings: {
...
listenChange(".file-validation", function () {
    ...
    const APP_IMAGE_MAX_BYTES = 5 * 1024 * 1024;

    if (window.APP_UPLOAD_MAX_BYTES < APP_IMAGE_MAX_BYTES &&
        file.size > window.APP_UPLOAD_MAX_BYTES) {

        const maxMB = Math.floor(window.APP_UPLOAD_MAX_BYTES / 1024 / 1024);

        displayErrorMessage(
            Lang.get('js.exceeds_allowed_upload_size', { size: maxMB })
        );
    ...
    if (file.size > APP_IMAGE_MAX_BYTES) {
        displayErrorMessage(
            Lang.get('js.image_too_large')
        );
        $(this).val("");
        return;
    }
...
function initializeColorPickers() {
    // Only initialize if color input elements exist
    if ($('#primary_color').length === 0) return;
