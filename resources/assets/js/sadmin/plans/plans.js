...
$(document).on('input change', '.trialDays', function () {
    var trialDaysValue = parseInt($(this).val()) || 0;

    if (trialDaysValue > 0) {
        $('input[name="price"]').val(0).prop("readonly", true);
    } else {
...
listenSubmit("#addPlanTaxForm", function (e) {
    e.preventDefault();
    $.ajax({
        url: route("plan.tax"),
        type: "POST",
...
listenClick("#newPlanTax", function () {
    $.ajax({
        url: route("plan.tax.get"),
        type: "GET",
        success: function (response) {
            $('#planTaxName').val(response.taxName);
            $('#planTax').val(response.tax);
            $('#planTaxStatus').prop('checked', response.status ?? false);

            $('#addPlanTaxModal').modal("show");
        },
        error: function () {
            displayErrorMessage(result.responseJSON.message);
        }
