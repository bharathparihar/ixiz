        ...
        <div class="modal-content" @if (getLanguage($vcard->default_language) == 'Arabic' || getLanguage($vcard->default_language) == 'Persian') dir="rtl" @endif>
            ...
            <div class="modal-body">
                ...
                @php
                    ...
                    $stripeEnabled = getUserSettingValue('stripe_enable', $vcard->user->id)
                        ? ($enabledPaymentMethods[1] = \App\Models\Product::STRIPE)
                        : null;
                    $paypalEnabled = getUserSettingValue('paypal_enable', $vcard->user->id)
                        ? ($enabledPaymentMethods[2] = \App\Models\Product::PAYPAL)
                        : null;
                    $flutterwaveEnabled = getUserSettingValue('flutterwave_enable', $vcard->user->id)
                        ? ($enabledPaymentMethods[7] = \App\Models\Product::FLUTTERWAVE)
                        : null;
                    $paystackEnabled = getUserSettingValue('paytack_enable', $vcard->user->id)
                        ? ($enabledPaymentMethods[5] = \App\Models\Product::PAYSTACK)
                        : null;
                    $manuallyEnabled = getUserSettingValue('manually_payment', $vcard->user->id)
                        ? ($enabledPaymentMethods[3] = \App\Models\Product::MANUALLY)
                        : null;
                    $phonepeEnabled = getUserSettingValue('phonepe_enable', $vcard->user->id)
                        ? ($enabledPaymentMethods[4] = \App\Models\Product::PHONEPE)
                        : null;
                    $razorpayEnabled = getUserSettingValue('rozorpay_enable', $vcard->user->id)
                        ? ($enabledPaymentMethods[6] = \App\Models\Product::RAZORPAY)
                        : null;
                    $payfastEnabled = getUserSettingValue('payfast_enable', $vcard->user->id)
                        ? ($enabledPaymentMethods[8] = \App\Models\Product::PAYFAST)
                        : null;
                    $mercadoPagoEnabled = getUserSettingValue('mercado_pago_enable', $vcard->user->id)
                        ? ($enabledPaymentMethods[9] = \App\Models\Product::MERCADOPAGO)
                        : null;
                        // $sslcommerzEnabled = getUserSettingValue('sslcommerz_enable', $vcard->user->id)
                        // ? ($enabledPaymentMethods[10] = \App\Models\Product::SSLCOMMERZ)
                        // : null;
                        $iyzicoEnabled = getUserSettingValue('iyzico_enable', $vcard->user->id)
                        ? ($enabledPaymentMethods[11] = \App\Models\Product::IYZICO)
                        : null;
                    $cashfreeEnabled = getUserSettingValue('cashfree_enable', $vcard->user->id)
                        ? ($enabledPaymentMethods[13] = \App\Models\Product::CASHFREE)
                        : null;
                    $translatedPaymentTypes = collect($enabledPaymentMethods)->map(function ($value) {
                        return $value !== null
                            ? trans('messages.' . \App\Models\Product::PAYMENT_METHOD[$value])
                            : null;
                    });
                @endphp
                <div class="mb-3">
                ...
                @if (!empty($vcard->privacy_policy) || !empty($vcard->term_condition))
                    <div class="form-check mb-3">
                        <input type="checkbox" name="product_terms" class="form-check-input terms-condition"
                            id="productTermCondition" placeholder>
                        <label class="form-check-label" for="privacyPolicyCheckbox">
                            <span>{{ __('messages.vcard.agree_to_our') }}</span>
                            <a href="{{ $vcardPrivacyAndTerm }}" target="_blank"
                                class="text-decoration-none link-info fs-6"
                                style="color: #0dcaf0 !important;">{!! __('messages.vcard.term_and_condition') !!}</a>
                            &
                            <a href="{{ $vcardPrivacyAndTerm }}" target="_blank"
                                class="text-decoration-none link-info fs-6"
                                style="color: #0dcaf0 !important;">{{ __('messages.vcard.privacy_policy') }}</a>
                        </label>
                    </div>
                @endif
                <div class="mt-3">
...
<script>
    if ("{{ getUserSettingValue('mercado_pago_enable', $vcard->user->id) }}" == "1") {
        var productMercadoPagoPublicKey = new MercadoPago(
