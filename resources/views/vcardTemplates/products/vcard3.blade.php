...
<body>
    <div class="container p-0">
        ...
        <div class="vcard-three main-content  w-100 mx-auto content-blur allSection collapse show">

            <div class="vcard-three__product_details_page py-3 mb-10 mt-0">
                <div class="row justify-content-between" @if (getLanguage($vcard->default_language) == 'Arabic' || getLanguage($vcard->default_language) == 'Persian') dir="rtl" @endif>
                    <div class="col-4">
                        <h4 class="vcard-three-heading p-5">{{ __('messages.vcard.products') }}</h4>
                    </div>
                    <div class="col-4 mt-6 text-center">
                        <a class="vcard-three-btn text-white text-decoration-none" href="{{ $vcardUrl }}"
                                    ...
                                    <div class="card product-card p-2 border-0 w-75 h-100">
                                        <div
                                            ...
                                            @foreach ($product->media as $media)
                                                <img src="{{ $media->getUrl() }}" alt="{{ $product->name }}"
                                                    class="text-center mt-1 p-2 object-fit-contain" height="208px"
                                                    loading="lazy">
                                            @endforeach
                                        </div>
                                        <div class="product-details mt-3">
                                            ...
                                            @if ($product->currency_id && $product->price)
                                                <span
                                                    class="text-white product-price-{{ $product->id }}">{{ currencyFormat($product->price, 2, $product->currency->currency_code) }}</span>
                                            @elseif($product->price)
                                                <span
                                                    class="text-white product-price-{{ $product->id }}">{{ currencyFormat($product->price, 2, getUserCurrencyIcon($vcard->user->id)) }}</span>
                                            @endif
                                </a>
                                <div class="d-flex justify-content-center">
                                @if (!empty($product->price))
                                    <div class="my-5 text-center">
                                        <button class="product-btn text-white rounded  buy-product"
                                            ...
                                            @if ($product->product_url)
                                                <div class="ms-1 my-5">
                                                    <a href="{{ $product->product_url ?? 'javascript:void(0)' }}"
                                                        ...
                                                        class="btn btn-product-enquiry rounded text-white @if(!$product->product_url) disabled @endif">
