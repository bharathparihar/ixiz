...
<head>
    ...
    <link rel="stylesheet" href="{{ mix('assets/css/vcard39.css') }}">
                            ...
                            <div class="product-card card mb-5">
                                ...
                                <div class="card-body p-3">
                                    <div class="product-desc d-flex justify-content-between align-items-center mt-5"
                                        @if (getLanguage($vcard->default_language) == 'Arabic') dir="rtl" @endif>
                                        <h3 class="text-primary fs-18 fw-5 mb-0 me-2">{{ $product->name }}</h3>
                                        <div class="product-amount fs-18">
                                            @if ($product->currency_id && $product->price)
                                                <span
                                                    class="fs-18 fw-semibold text-white  product-price-{{ $product->id }}">{{ $product->currency->currency_icon }}{{ number_format($product->price, 2) }}</span>
                                            @elseif($product->price)
                                                <span
                                                    class="fs-18 fw-semibold text-white  product-price-{{ $product->id }}">{{ getUserCurrencyIcon($vcard->user->id) }}{{ $product->price }}</span>
                                            @endif
                                        </div>
                                    </div>
                                    <p class="fs-14 mb-0 py-5 ps-2 prod-content-desc">{{ $product->description }}</p>
