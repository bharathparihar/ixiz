...
<body>
    <div class="container p-0">
        <div class="vcard-ten main-content  w-100 mx-auto content-blur allSection collapse show">

            <div class="vcard-ten__product_details_page py-3 mt-0">
                <div class="row justify-content-between" @if (getLanguage($vcard->default_language) == 'Arabic' || getLanguage($vcard->default_language) == 'Persian') dir="rtl" @endif>
                    <div class="col-4">
                        <h4 class="vcard-ten-heading p-5">{{ __('messages.vcard.products') }}</h4>
                    </div>
                    <div class="col-4 mt-6 text-center">
                        <a class="product-btn text-decoration-none" href="{{ $vcardUrl }}"
                                    ...
                                    <div class="card product-card p-2 border-0 w-75 h-100">
                                        ...
                                        <div class="product-details mt-3">
                                            ...
                                            @if ($product->currency_id && $product->price)
                                                <span
                                                    class="product-price-{{ $product->id }}">{{ currencyFormat($product->price, 2, $product->currency->currency_code) }}</span>
                                            @elseif($product->price)
                                                <span
                                                    class="product-price-{{ $product->id }}">{{ currencyFormat($product->price, 2, getUserCurrencyIcon($vcard->user->id)) }}</span>
                                            @endif
                                </a>
                                <div class="d-flex justify-content-center">
                                @if (!empty($product->price))
                                    <div class="my-5 text-center ">
                                        <button class="product-btn text-decoration-none rounded-2 buy-product"
                                            ...
                                            @if ($product->product_url)
                                                <div class="ms-1 my-5">
                                                    <a href="{{ $product->product_url ?? 'javascript:void(0)' }}"
                                                        ...
                                                        class="product-btn text-decoration-none rounded-2  @if(!$product->product_url) disabled @endif">
