                                            ...
                                            <div class="d-flex ">
                                                ...
                                                <div class="d-flex flex-wrap">
                                                    @if($vcard->phone)
                                                        ...
                                                        class="ps-2 fs-14" dir="ltr">+ {{ $vcard->region_code }}
                                                            {{ $vcard->phone }}</a>
                                                    @endif
                                                    @if($vcard->alternative_phone)
                                                        <a href="tel:+{{ $vcard->alternative_region_code }}{{ $vcard->alternative_phone }}"
                                                        class="ps-2 fs-14" dir="ltr">+ {{ $vcard->alternative_region_code }}
                                    ...
                                    <div class="card-body p-0 ps-sm-4 pt-sm-0 pt-3">
                                        <a class="text-decoration-none text-white"
                                           href="{{ $service->service_url ?? '#' }}" target="_blank">
                                            ...
                                            {{--                                        @endphp--}}
                                            <p class="card-text fs-14  mb-0 {{ \Illuminate\Support\Str::length($service->description) > 170 ? 'more' : ''}}">
        ...
        <!-- start product section -->
                ...
                @if ((isset($managesection) && $managesection['products']) || empty($managesection))
                    <section class="services-section pt-30">
                        <div class="section-heading mb-5">
                            ...
                            <div class="text-end ">
                                <a class="fs-6 ps-4 text-decoration-underline text-light" href="{{ $vcardProductUrl }}">{{__('messages.analytics.view_more')}}</a>
                                </div>
                        </div>
                        <?php $ProductCount = 1 ?>
                        <div class="row">
                                    ...
                                    <a @if($product->product_url) href="{{ $product->product_url }}" @endif >
                                        <div class="card flex-sm-row p-sm-4 p-3 h-100">
                                            ...
                                            <div class="card-body p-0 ps-sm-4 pt-sm-0 pt-3">
                                                ...
                                                <h5 class="card-title text-white fs-18">{{ $product->name }}</h5>
                                                    </div>
                                                @if($product->currency_id && $product->price)
                                                    <p class="text-white fs-14 pb-4 mb-0">
                                                        {{$product->currency->currency_icon}}{{getSuperAdminSettingValue('hide_decimal_values') == 1 ? number_format($product->price, 0) : number_format($product->price, 2)}}
                                                    </p>
                                                @elseif($product->price)
                                                    <p class=" text-white fs-14 pb-4 mb-0">{{ getUserCurrencyIcon($vcard->user->id) .' '. $product->price }}</p>
                                                @endif
                                            <div class="">
                                                <p class="card-text text-white fs-14 pb-4 mb-0">
                                                    {!! $product->description !!}
                                                </p>
                                            </div>
                                            @if ($vcard->show_service_enquiry_btn == true)
            ...
            @if ((isset($managesection) && $managesection['whatsapp_store']) || empty($managesection))
                @if ($whatsappStore->count())
                    <section class="services-section pt-30 mt-20">
                        <div class="section-heading mb-5">
                            <h2 class="fs-22 text-white ps-4">
                            ...
                            @foreach ($whatsappStore as $store)
                                <div class="col-md-6 mb-sm-5 mb-4">
                                    ...
                                    <div class="card flex-sm-row p-sm-4 p-3 h-100">
                                        ...
                                        <div class="card-body p-0 ps-sm-4 pt-sm-0 pt-3 text-center text-sm-start"
                                            @if (getLanguage($vcard->default_language) == 'Arabic' || getLanguage($vcard->default_language) == 'Persian') dir="rtl" @endif>

                                            <div class="d-flex justify-content-between align-items-center">
                                                <h5 class="card-title fs-18 text-white mb-2 whatsapp-store-name">
                                                    {{ $store->store_name }}
                                                </h5>
                                            </div>
                                            <div class="text-end mt-3">
                                                <a class="btn btn-primary btn-sm" target="_blank" href="{{ route('whatsapp.store.show', $store->url_alias) }}">
        ...
        <!-- end services section -->
            ...
            @if ((isset($managesection) && $managesection['payment_link']) || empty($managesection))
                @if (checkFeature('payment_link') && $vcard->paymentLinks->count())
                    <section class="services-section pt-30">
                        <div class="section-heading mb-5">
                            <h2 class="fs-22 text-white ps-4">{{ __('messages.vcard.payment_links') }}</h2>
                        </div>
                        <?php $paymentLinkCount = 1; ?>
                        <div class="row">
                            @foreach ($vcard->paymentLinks as $paymentLink)
                                ...
                                <div class="col-md-6 mb-sm-5 mb-4">
                                    <div class="card flex-sm-row p-sm-4 p-3 h-100">
                                        <div class="tag d-flex justify-content-center align-items-center">
                                            <span class="fs-6 text-white">{{ $paymentLinkCount++ }}</span>
                                        </div>
                                        <div class="card-img-top">
                                            <img src="{{ $iconUrl }}"
                                                class="w-100 h-100 object-fit-cover custom-border-radius"
                                        ...
                                        <div class="card-body p-0 ps-sm-4 pt-sm-0 pt-3">
                                            <div class="d-flex justify-content-between align-items-start">
                                                <h5 class="card-title fs-18 pl-strip-title">{{ $paymentLink->label }}</h5>
                                                <div class="ms-2">
                                                    @if ($paymentLink->display_type == \App\Models\VcardPaymentLink::UPI)
                                                        <a href="{{ $linkUrl }}"
                                                            class="btn btn-primary-enquiry btn-sm js-upi-pay d-inline-block d-md-none"
                                                            style="padding: 4px 15px; font-size: 0.85rem;"
                                                            data-upi-url="{{ $linkUrl }}"> {{ __('messages.vcard.pay') }} </a>
                                                    @elseif($paymentLink->display_type == \App\Models\VcardPaymentLink::LINK)
                                                        <a href="{{ $linkUrl }}" target="_blank"
                                                            class="btn btn-primary-enquiry btn-sm mt-0 mt-md-4"
                                                            style="padding: 7px 12px 5px 15px; font-size: 0.95rem;">
                                                            <i class="fa-solid fa-arrow-up-right-from-square me-1"></i>
                                                        </a>
                                                    @elseif($isImageType)
                                                        <img src="{{ $linkUrl }}" alt="{{ $paymentLink->label }}"
                                                            class="img-fluid rounded pl-strip-content-img" style="max-height: 85px;">
                                                    @endif
                                                </div>
                                            </div>
                                            @if (
                                                ...
                                                <p class="card-text fs-14 mb-0 description-text">
                        ...
                        <div class="modal-dialog news-modal modal-dialog-centered">
                            <div class="modal-content animate-bottom" id="newsLatter-content">
                                <div class="newsmodal-header px-0 position-relative">
                                    <button type="button" class="btn-close p-1 position-absolute end-0" data-bs-dismiss="modal"
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mb-1 mt-3 d-flex gap-1 justify-content-center align-items-center email-input">
                                        ...
                                        <input type="email" class="form-control email-input w-100" placeholder="{{ __('messages.form.enter_your_email') }}" aria-label="Email" aria-describedby="button-addon2">
                                        <button class="btn btn-outline-secondary ms-1" type="button" id="email-send">{{ __('messages.subscribe') }}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
