...
<body>
    <div class="container p-0">
        <div class="vcard-twentysix main-content  w-100 mx-auto content-blur allSection collapse show">
            <div class="vcard-one__product py-3 mt-0">
                <div class="row justify-content-between" @if (getLanguage($vcard->default_language) == 'Arabic' || getLanguage($vcard->default_language) == 'Persian') dir="rtl" @endif>
                    <div class="col-4">
                        <h4 class="vcard-twentysix-heading p-5">{{ __('messages.vcard.products') }}</h4>
                    </div>
                    <div class="col-4 mt-6 text-center">
                        <a class="back-btn text-decoration-none text-dark"
                            href="{{ $vcardUrl }}"
                ...
                <div class="product-slider">
                    @foreach ($vcard->products as $product)
                        <div class="products">
                            <div class="product-card card mb-10 pt-5">
                                <div class="product-img card-img">
                                    <a @if ($product->product_url) href="{{ $product->product_url }}" @endif
                                        target="_blank" class="text-decoration-none fs-6">
                                        <div class=" {{ $product->media->count() < 2 ? 'd-flex justify-content-center' : '' }} product-img-slider overflow-hidden">
                                            @foreach ($product->media as $media)
                                                <img src="{{ $media->getUrl() }}" alt="{{ $product->name }}"
                                                    class="text-center mt-0 pb-2 object-fit-contain rounded-2" height="208px"
                                                    loading="lazy">
                                            @endforeach
                                        </div>
                                    </a>
                                    ...
                                    <div class="product-desc d-flex justify-content-between align-items-center"
                                        ...
                                        <div class="product-amount text-primary fw-bold fs-18">
                                            @if ($product->currency_id && $product->price)
                                                <span
                                                    class="fs-18 fw-6 text-dark product-price-{{ $product->id }}">{{ currencyFormat($product->price, 2, $product->currency->currency_code) }}</span>
                                            @elseif($product->price)
                                                <span
                                                    class="fs-18 fw-6 text-dark product-price-{{ $product->id }}">{{ currencyFormat($product->price, 2, getUserCurrencyIcon($vcard->user->id)) }}</span>
                                            @endif
                                        </div>
                                    </div>
