...
<head>
    ...
    <link rel="stylesheet" href="{{ mix('assets/css/oldVcard/vcard1.css') }}">
...
<body>
    <div class="container p-0 product-details-page">
        <div class="vcard-eighteen bg-light main-content mx-auto w-100 overflow-hidden border">
            ...
            <div class="vcard-one__product py-3 mb-10 mt-0">
                <div class="row justify-content-between" @if (getLanguage($vcard->default_language) == 'Arabic' || getLanguage($vcard->default_language) == 'Persian') dir="rtl" @endif>
                    <div class="col-4">
                        <h4 class="vcard-prodcut-heading fw-500 fs-1 p-2 text-primary">
                            {{ __('messages.vcard.products') }}</h4>
                    </div>
                    <div class="col-4 mt-20 p-4 text-center">
                        <a class="vcard-eighteen-btn text-white bg-primary text-decoration-none"
                            href="{{ $vcardUrl }}"
                        ...
                        @foreach ($products as $product)
                            <div class="d-flex justify-content-center mb-2">
                                <a @if ($product->product_url) href="{{ $product->product_url }}" @endif
                                    ...
                                    <div class="card product-card border w-75 h-100">
                                        <div class="{{ $product->media->count() < 2 ? 'd-flex justify-content-center dots-none' : '' }} product-img-slider overflow-hidden mt-1">
                                            @foreach ($product->media as $media)
                                            <div>
                                                <div class="product-img-height h-100 text-center p-2 w-100 mx-auto">
                                        ...
                                        <div class="bg-primary-light card-body">
                                            ...
                                            <p class="text-dark mb-0 p-2">
                                                {{ $product->description }}</p>
                                </a>
                                <div class="d-flex justify-content-center">
                                @if (!empty($product->price))
                                    <div class="mt-2 text-center ">
                                        <button class="btn send-btn rounded-2 btn-primary buy-product w-50"
                                            ...
                                            @if ($product->product_url)
                                                <div class="ms-1 mt-2">
                                                    <a href="{{ $product->product_url ?? 'javascript:void(0)' }}"
                                                        ...
                                                        class="btn send-btn btn-primary rounded-2 btn-product-enquiry text-white @if(!$product->product_url) disabled @endif">
