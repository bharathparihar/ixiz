...
class SubscriptionController extends AppBaseController
{
    ...
    public function choosePaymentType($planId, $context = null, $fromScreen = null)
    {
        ...
        $planTaxEnabled = (bool) Setting::where('key', 'plan_tax_enabled')->value('value');
        $planTaxValue = (float) Setting::where('key', 'plan_tax_value')->value('value');

        $basePrice = $customField
        ? (float) $customField->custom_vcard_price
        : (float) $subscriptionsPricingPlan->price;

        /** REMAINING BALANCE */
        $cpData = getCurrentPlanDetails();
        $remainingBalance = (float) ($cpData['remainingBalance'] ?? 0);

        /** AFTER REMAINING */
        $afterRemaining = max($basePrice - $remainingBalance, 0);

        /** COUPON (already applied via backend) */
        $couponDiscount = (float) ($cpData['couponDiscount'] ?? 0);
        $afterCoupon = max($afterRemaining - $couponDiscount, 0);

        /** TAX (LAST STEP) */
        $taxAmount = 0;
        if ($planTaxEnabled && $planTaxValue > 0 && $afterCoupon > 0) {
            $taxAmount = round(($afterCoupon * $planTaxValue) / 100, 2);
        }

        /** FINAL PAYABLE */
        $totalAmount = round($afterCoupon + $taxAmount, 2);
        $paymentTypes = getPaymentGateway();


        return view('subscription.payment_for_plan', compact('subscriptionsPricingPlan', 'paymentTypes', 'customSelectId', 'customField','planTaxEnabled', 'planTaxValue', 'taxAmount', 'totalAmount', 'taxName'));
    }
