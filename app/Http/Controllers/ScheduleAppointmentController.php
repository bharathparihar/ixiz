...
class ScheduleAppointmentController extends AppBaseController
{
        ...
        try {
            ...
            if (isset($input['payment_method'])) {
                ...
                if ($input['payment_method'] == Appointment::MERCADOPAGO) {
                    ...
                    $result = App::make(MercadoPagoController::class)->appointmentOnBoard($userId, $vcard, $input);
                ...
                //payfast
                if ($input['payment_method'] == Appointment::PAYFAST) {
                    $currency  = $input['currency_code'];
                    if ($currency != "ZAR") {
                    ...
                    $result = $payfast->appointmentBook($userId, $vcard, $input);
                ...
                //iyzico
                if ($input['payment_method'] == Appointment::IYZICO) {
                    ...
                    $url = $iyzico->appointmentBook($vcard, $input, $currency);

                    if (!filter_var($url, FILTER_VALIDATE_URL)) {
                        return $this->sendError($url);
                    }
                ...
                // Sslcommerz
                // if ($input['payment_method'] == Appointment::SSLCOMMERZ) {
                //     $currencyCode = $input['currency_code'];
                //     if ($currencyCode != "BDT") {
                ...
                //         return $this->sendError(__('messages.placeholder.please_add_payment_credentials'));
                //     }
                ...
                //             'vcard_alias' => $vcard->url_alias,
                //         ]),
                //     ]);

                //     $response = Sslcommerz::setOrder($amount, $invoice, 'Appointment Payment')
                ...
                if ($input['payment_method'] == Appointment::CASHFREE) {
                    $currency  = $input['currency_code'];
                    if ($currency != "INR") {
                    ...
                    $result = $cashfree->appointmentBookCashfree($userId, $vcard, $input);
                    ...
                    return response()->json([
                        ...
                        'data' => [
                            'payment_method' => Appointment::CASHFREE,
            ...
            $appointmentRepo->appointmentStoreOrEmail($input, $vcardEmail, $vcard->default_language, $vcard->url_alias);
    ...
    public function paymentStatus(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $appointmentTran = AppointmentTransaction::find($appointmentTranId);

            if ($appointmentTran->type !== $appointmentStatus) {
                $appointmentTran->update([
                    'type' => $appointmentStatus,
                ]);

                if ($appointmentStatus === 5) {
                    $input = [
                        'name' => $appointmentTransaction->name ?? 'User',
                        'email' => $appointmentTransaction->email,
                        'date' => $appointmentTransaction->appointment_date,
                        'from_time' => $appointmentTransaction->from_time,
                        'to_time' => $appointmentTransaction->to_time,
                    ];

                    if (!empty($input['email'])) {
                        Mail::to($input['email'])->send(new AppointmentApproveMail($input));
                    }
                }
            }
