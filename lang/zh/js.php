<?php

return [
    'last_name_required' => '姓氏字段是必填的',
    'first_name_required' => '名字字段是必填的',
    'email_required' => '电子邮件字段是必填的',
    'invalid_email' => '电子邮件无效',
    'coupon_code_apply' => '优惠码已成功应用.',
    'product' => '产品',
    'vcard_testimonial' => 'V卡推荐',
    'vcard_iframe' => 'V卡 iframe',
    'vcard_service' => 'V卡服务',
    'payment_link' => '支付链接',
    'payment_link_description' => '描述',
    'image' => '图像',
    'upi_tooltip' => 'UPI链接格式:
                    upi://pay?pa=VPA@bank&am=Amount&cu=INR
                    参数:
                    pa = 收款人UPI ID (例如 9876543210@ybl)
                    am = 金额 (例如 100 或 1.50)
                    cu = 货币 (INR)
                    示例:
                    upi://pay?pa=9876543210@ybl&am=100&cu=INR
                    提示: 删除am以让客户手动输入金额',
    'approved' => '已批准',
    'rejected' => '已拒绝',
    'in_progress' => '进行中',
    'vcard' => '名片',
    'check_email' => '電子郵件已經存在。',
    'affiliation_amount_error' => '隶属金额不能超过100%',
    'yesterday' => '昨天',
    'last_7_days' => '最近7天',
    'last_30_days' => '最近30天',
    'this_month' => '本月',
    'copied_successfully' => '链接已成功复制。',
    'today' => '今天',
    'month' => '月',
    'vcard_name' => 'vCard名称',
    'email' => '电子邮件',
    'phone' => '电话',
    'user' => '用户',
    'appointment' => '约会',
    'appointments' => '约会',
    'completed' => '已完成',
    'has_been_completed' => '已完成。',
    'are_you_completed' => '您确定要批准吗？',
    'Yes_Change' => '是的，批准',
    'no' => '不',
    'agree_term' => '您必须同意条款和条件。',
    'coupon_code_percent_validation' => '优惠券折扣不得超过100％。',
    'invalid_number' => '无效号码',
    'custom' => '自定义',
    'subscriptions' => '订阅',
    'multi_templates' => '需要多个模板',
    'testimonial' => '推荐',
    'faqs' => '常见问题',
    'app_name_required' => '应用程序名称字段是必需的',
    'purchase' => '购买',
    'first_name_required' => '名字字段是必需的。',
    'choose_one_template' => '选择任何一个模板',
    'select_hour' => '请选择日期或小时',
    'social_links_is_required' => '社交链接是必需的。',
    'payment_not_complete' => '支付未完成',
    'yes' => '是',
    'delete' => '删除',
    'are_you_sure' => '您确定要删除吗',
    'admin' => '管理员',
    'coupon_code' => '优惠券代码',
    'flat' => '平',
    'successful' => '成功',
    'error' => '错误',
    'deleted' => '已删除',
    'has_been_deleted' => '已被删除。',
    'ok' => '好',
    'first_name_required' => '名字字段是必需的。',
    'last_name_required' => '姓氏字段是必需的。',
    'enter_valid_email' => '请输入有效的电子邮件。',
    'allowed_file_types' => '允许的文件类型：png，jpg，jpeg。',
    'invalid_country_code' => '无效的国家代码',
    'too_short' => '太短',
    'too_long' => '太长',
    'apply' => '应用',
    'cancel' => '取消',
    'from' => '从',
    'to' => '到',
    'jan' => '一月',
    'feb' => '二月',
    'mar' => '三月',
    'apr' => '四月',
    'may' => '五月',
    'jun' => '六月',
    'jul' => '七月',
    'aug' => '八月',
    'sep' => '九月',
    'oct' => '十月',
    'nov' => '十一月',
    'dec' => '十二月',
    'sun' => '星期日',
    'mon' => '星期一',
    'tue' => '星期二',
    'wed' => '星期三',
    'thu' => '星期四',
    'fri' => '星期五',
    'sat' => '星期六',
    'this_week' => '本周',
    'last_week' => '上周',
    'amount' => '数量',
    'month' => '月',
    'select_one_or_more' => '选择一个或多个功能是必需的',
    'phone_number_required' => '电话号码字段是必需的。',
    'plan_expire_notification' => '计划到期通知字段是必需的。',
    'address_field' => '地址字段是必需的。',
    'stripe_key' => 'Stripe密钥字段是必需的。',
    'stripe_secret' => 'Stripe秘密字段是必需的。',
    'paypal_secret' => 'Paypal秘密字段是必需的。',
    'paypal_key' => 'Paypal 客户 ID 字段为必填项。',
    'paypal_mode' => 'Paypal模式字段是必需的。',
    'paystack_key' => 'Paystack密钥字段是必需的。',
    'paystack_secret' => 'Paystack秘密字段是必需的。',
    'flutterwave_key' => 'Flutterwave密钥字段是必需的。',
    'flutterwave_secret' => 'Flutterwave秘密字段是必需的。',
    'razorpay_key' => 'Rozorpay 关键字段是必需的。',
    'razorpay_secret' => 'Rozorpay 秘密字段是必需的。',
    'onesignal_app_id' => 'OneSignal 应用 ID 字段为必填项。',
    'onesignal_rest_api_key' => 'OneSignal REST API 密钥字段为必填项。',
    'term_condition' => '条款和条件',
    'privacy_policy' => '隐私政策',
    'manual_payment_guide' => '手动支付指南',
    'loading' => '加载中...',
    'choose_plan' => '选择计划',
    'remove' => '移除',
    'passwords' => '密码字段是必需的。',
    'password_character' => '密码必须至少为8个字符。',
    'password_must_match' => '密码和密码确认必须匹配。',
    'description' => '输入您的vCard描述',
    'the_term_conditions' => '需要接受条款和条件。',
    'unavailable' => '不可用',
    'duplicate' => '重复！',
    'are_you_sure_dublicate_vcard' => '您确定要创建此vCard的副本吗？',
    'duplicate_vcard' => '重复vCard！',
    'duplicate_vcard_create' => '成功创建重复vCard',
    'upload_required' => '上传文件字段是必需的。',
    'gallery' => '画廊',
    'social_links_img_is_required' => '社交链接图片是必需的。',
    'email_required' => '电子邮件字段为必填项',
    'allowed_image' => '允许的文件类型：png，jpg，jpeg。',
    'phonepe_merchant_id_required' => 'Phonepe商户ID字段是必填的.',
    'phonepe_merchant_user_id_required' => 'Phonepe商户用户ID字段是必填的.',
    'phonepe_env_required' => 'Phonepe环境字段是必填的.',
    'phonepe_salt_key_required' => 'Phonepe盐值密钥字段是必填的.',
    'phonepe_merchant_transaction_id_required' => 'Phonepe商户交易ID字段是必填的.',
    'phonepe_salt_index_required' => 'Phonepe盐值索引字段是必填的.',
    'click_to_go_dashboard' => '单击“下一步”进入仪表板.',
    'this_is_analytics_chart' => '这是您的 vCard 分析图表.',
    'click_open_sidebar' => '单击“下一步”打开侧边栏.',
    'click_to_make_vcards' => '单击“下一步”制作新的电子名片.',
    'click_to_create_vcards' => '单击“下一步”创建您的电子名片.',
    'click_to_generate_alias' => '单击此处生成您的网址别名.',
    'click_to_save_vcard' => '单击“下一步”保存您的电子名片.',
    'click_to_open_edit_sidebar' => '单击“下一步”访问 vCard 编辑侧栏.',
    'here_select_vcard_template' => '您可以在此处选择您的电子名片模板.',
    'click_to_preview' => '单击此处预览您的电子名片',
    'next' => '下一个',
    'skip' => '跳过',
    'generate_url_alias' => '请生成一个 URL 别名',
    'enter_vcard_name' => '请输入电子名片名称',
    'inquries' => '询问',
    'send_email_description' => '描述',
    'description_required' => '描述字段是必填项.',
    'select_email_required' => '发送邮件字段是必填项.',
    'subject_required' => '主题字段是必填项.',
    'blog_description' => '描述',
    'blog_image_required' => '部落格圖像欄位是必需的。',
    'enquiry' => '查询',
    'blog' => '博客',
    'city' => '城市',
    'country' => '国家',
    'state' => '州/省',
    'nfc_card' => 'NFC 卡',
    'plan' => '计划',
    'custom_link' => '自定义链接',
    'select_all' => '选择全部',
    'select_payment_type' => '选择付款方式',
    'please_wait' => '请稍等...',
    'image_too_large' => '您的文件太大。允許的最大大小為 5MB。',
    'select_user' => '选择用户',
    'please_select_user' => '请选择一个用户',
    'select_vcard' => '选择电子名片',
    'add_on' => '附加',
    'embedtag' => '嵌入标签',
    'short_description' => '请输入简短描述',
    'description_length' => '描述必须至少包含3个字符。',
    'product_category' => '产品类别',
    'whatsapp_store' => 'WhatsApp 商店',
    'product_added_to_cart' => '产品已成功添加到您的购物车！',
    'product_deleted_from_cart' => '产品已成功从您的购物车中移除！',
    'order_placed' => '您的订单已成功下单！',
    'affiliation_email_send' => '邀请邮件已成功发送！',
    'nfc_order' => 'NFC 订单',
    'email_template_content' => '电子邮件模板内容',
    'email_template_content_required' => '电子邮件模板内容字段是必填项。',
    'email_template_subject_required' => '电子邮件模板主题字段是必填项。',
    'customer_details' => '客户详情',
    'name' => '姓名',
    'address' => '地址',
    'order_id' => '订单编号',
    'product_details' => '产品详情',
    'product_url' => '产品链接',
    'price' => '价格',
    'quantity' => '数量',
    'total_price' => '总价',
    'grand_total' => '总计',
    'product_name' => '产品名称',
    'order_dispatched' => '您的订单已发货',
    'order_cancelled' => '您的订单已取消',
    'order_delivered' => '您的订单已送达',
    'no_more_quantity' => '此产品暂无库存',
    'item_not_addded_to_cart' => '商品未添加到购物车',
    'payfast_key' => 'Payfast 商户密钥字段是必填项。',
    'payfast_secret' => 'Payfast 商户 ID 字段是必填项。',
    'invalid_email_domain' => '无效的电子邮件域名',
    'duplicate_email_domain' => '重复的电子邮件域名',
    'entering_multiple_domain_must_be_comma_separated' => '如果输入多个域名，必须使用逗号分隔（例如：gmail.com, outlook.com）',
    'whatsapp_product_order' => 'WhatsApp 产品订单',
    'sslcommerz_StoreId' => 'Sslcommerz 商店 ID 字段是必填项。',
    'sslcommerz_Password' => 'Sslcommerz 商店密码字段是必填项。',
    'iyzico_key' => 'Iyzico 密钥字段是必填项',
    'iyzico_secret' => 'Iyzico 密码字段是必填项',
    'open_ai_key' => 'Open AI API 密钥字段为必填项。',
    'open_ai_model' => 'Open AI 模型字段为必填项。',
    'please_enter_prompt' => '请输入提示。',
    'description_generated_successfully' => '描述生成成功！',
    'are_you_sure_want_to_delete_selected_Users' => '您确定要删除所选用户吗？',
    'please_select_one_or_more_records_for_multiples_delete' => '请选择一个或多个记录进行批量删除。',
    'package_added_to_cart' => '套餐已成功添加到您的购物车！',
    'package_deleted_from_cart' => '套餐已成功从您的购物车中移除！',
    'no_more_quantity_package' => '该套餐没有更多数量可用',
    'custom_page' => '自定义页面',
    'invalid_youtube_url' => '请输入有效的 YouTube 链接',
    'youtube_link_required' => '必须填写 YouTube 链接',
    'duplicate_youtube_links' => '不允许重复的 YouTube 链接',
    'enter_youtube_video_link' => 'Nhập liên kết video YouTube',
    'pending' => '待处理',
    'dispatched' => '已发货',
    'delivered' => '已送达',
    'cancelled' => '已取消',
    'mp_public_key' => '必须填写 MercadoPago 公钥字段',
    'mp_access_token' => '必须填写 MercadoPago 访问令牌字段',
    'refund_cancellation' => '退款与取消',
    'shipping_delivery' => '运输与交付',
    'the_privacy_policy' => '隐私政策是必需的。',
    'the_refund_cancellation' => '退款与取消政策是必需的。',
    'the_shipping_delivery' => '运输与交付政策是必需的。',
    'crop' => '裁剪',
    'crop_image' => '裁剪图片',
    'cashfree_key' => '必须填写 Cashfree 应用 ID 字段。',
    'cashfree_secret' => '必须填写 Cashfree 密钥字段。',
    'service_name' => '服务名称',
    'select_service' => '选择服务',
    'service' => '服务',
    'this_service_is_already_selected' => '该服务已被选择。',
    'exceeds_allowed_upload_size' => '所选图片超过允许的上传大小。请上传小于 :size MB 的图片。',
];
